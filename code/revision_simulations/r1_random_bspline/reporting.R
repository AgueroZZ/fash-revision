...
if (length(missing_cache_paths) > 0L) {
  stop(
    "The formal R1 linear-mixture cache is incomplete. Missing: ",
...
r1_reporting_overlay <- load_r1_r2_fashr0143_overlay(
  scenario_id = "r1",
...
out <- r1_reporting_overlay$out
fash_alpha <- r1_reporting_overlay$fash_alpha
fash_alpha_005 <- r1_reporting_overlay$fash_alpha_005
direct_alpha <- r1_reporting_overlay$direct_alpha
direct_alpha_005 <- r1_reporting_overlay$direct_alpha_005
pi0_summary <- r1_reporting_overlay$pi0_summary
linear_prior_weights <- r1_reporting_overlay$linear_prior_weights
linear_prior_summary <- r1_reporting_overlay$linear_prior_summary
genotype_selection_summary <-
  r1_reporting_overlay$genotype_selection_summary
truth_maf_balance <- r1_reporting_overlay$truth_maf_balance
result_provenance_table <- r1_reporting_overlay$provenance_table
current_fash_manifest <- r1_reporting_overlay$manifest
...
expected_J <- 6362L
expected_class_counts <- exact_proportional_counts(
  expected_J,
...
if (!identical(configuration$output_id, expected_output_id) ||
    ...
    !identical(
      as.integer(configuration$common_penalty),
      expected_penalty
    ) ||
...
if (nrow(genotype_selection_summary) != length(expected_seed_list) ||
    ...
    nrow(truth_maf_balance) !=
      length(expected_seed_list) * length(expected_class_probs) ||
    !setequal(truth_maf_balance$seed, expected_seed_list) ||
    !setequal(truth_maf_balance$effect_class, names(expected_class_probs)) ||
...
weights_valid <- length(prior_weight_groups) == length(expected_seed_fit_keys) &&
  ...
  all(vapply(prior_weight_groups, function(rows) {
    nrow(rows) == length(expected_sd_grid) &&
      isTRUE(all.equal(rows$predstep_sd, expected_sd_grid, tolerance = 1e-12)) &&
