...
# Run the resumable R3 ideal-Gaussian measurement isolation experiment.
# The frozen R3 truth and t-adjusted standard errors are regenerated exactly;
# only beta_hat is replaced by a Gaussian draw conditional on the fixed SE.
...
SCHEMA_VERSION <- "r3-ideal-gaussian-measurement-v1"
...
EXPECTED_TRUTH_GROUP_COUNTS <- stats::setNames(
  rep(212L, 6L), TRUTH_GROUP_LEVELS
)
...
configuration <- list(
  ...
  temporal_category_design = "equal temporal categories",
...
if (PREFLIGHT_ONLY) {
  log_message(
    ...
    "seed separation, and scientific configuration."
  )
