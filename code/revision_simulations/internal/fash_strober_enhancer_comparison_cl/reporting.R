# Reporting helpers for the CL-PC R6 enhancer enrichment page.
#
# The page reports enhancer annotations only, for FASH-CL against the two
# Strober analyses, in two sections: an unfiltered method comparison and a
# variant-level FASH-CL-only view. It deliberately does not surface BH q-values;
...
if (!file.exists(cache_path)) {
  stop(
    ...
    "code/revision_simulations/internal/fash_strober_enhancer_comparison_cl/",
    "run_fash_strober_enhancer_comparison_cl.R first."
  )
}
...
expected_cache_id <- "fash_strober_enhancer_comparison_cl_fashr0143"
...
expected_cl_adjustment_path <- normalizePath(file.path(
  workflowr_root,
  "output",
  "revision_simulations",
  "internal",
...
cl_adjustment_provenance <- cache$input_provenance[
  cache$input_provenance$path == expected_cl_adjustment_path,
  ,
  drop = FALSE
]
if (nrow(cl_adjustment_provenance) != 1L ||
    cl_adjustment_provenance$md5 != "ddafd704c78ab906fe1259fa8d887e1f" ||
    cl_adjustment_provenance$sha256 !=
      "03a95943d8b000c41fc7d8cd50062edabad4b4b9b14ffc22112530a87197e4a0") {
  stop("The corrected BF-updated FASH-CL input failed provenance validation.")
}
...
if (anyNA(discovery_order) ||
  ...
  stop("The corrected CL-PC R6 discovery-set counts failed validation.")
}
...
# Section 2: the FASH-CL-only set, with the unfiltered focal set drawn behind it
# grey so the reader can see what the exclusion actually changed.
plot_fash_only <- function() {
  exclusive_label <- "FASH-CL only (Strober rsIDs removed)"
  reference_label <- "FASH-CL, all discoveries (reference)"
  ...
  reference <- enhancer_results[
    enhancer_results$section == 1L &
      enhancer_results$method == "FASH-CL",
    ,
    drop = FALSE
  ]
  if (!nrow(exclusive) || !nrow(reference)) {
    stop("FASH-CL-only plot data is missing.")
  }
  exclusive$series <- exclusive_label
  reference$series <- reference_label
  colors <- stats::setNames(
    c(unname(method_colors[["FASH-CL"]]), reference_color),
...
lead_gene_count <- discovery_set_summary$unique_gene_count[
  discovery_set_summary$discovery_set == "cl_lead"
]
