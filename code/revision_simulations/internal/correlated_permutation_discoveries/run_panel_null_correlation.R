...
# ---------------------------------------------------------------------------
# PATCHED SELECTION. Everything else in this script, in particular the
# regression and permutation core, is byte-identical to
# covariance_estimation/run_multigene_null_beta_covariance_exploration.R.
#
# The original selects genes by a gene-level minimum-lfdr threshold. This
# variant runs the same null on an arbitrary supplied panel, so that the
# threshold panel can be compared against a panel chosen without reference to
# any outcome, and against the units the correlated FASH fit discovered.
#
#   --panel threshold  genes with gene-level min lfdr > 0.90 (the original)
#   --panel random     genes drawn with no threshold at all
#   --panel discovery  the pair keys listed in --pair-keys
# ---------------------------------------------------------------------------
panel <- as.character(get_arg("--panel", "random"))
panel_size <- as.integer(get_arg("--panel-size", "874"))
panel_seed <- as.integer(get_arg("--panel-seed", "20260823"))
pair_key_path <- get_arg("--pair-keys", NA_character_)
if (!panel %in% c("threshold", "random", "discovery")) {
  stop("--panel must be threshold, random, or discovery.")
}

catalog <- make_randomized_pair_catalog(pair_keys, seed = 20260809L)

if (identical(panel, "discovery")) {
  if (is.na(pair_key_path) || !file.exists(pair_key_path)) {
    stop("--pair-keys must point to a CSV with a pair_key column.")
  }
  requested <- utils::read.csv(pair_key_path, stringsAsFactors = FALSE)$pair_key
  requested <- unique(as.character(requested))
  missing <- setdiff(requested, pair_keys)
  if (length(missing) > 0L) {
    stop(length(missing), " requested pair keys are not in the FASH fit.")
  }
  selected <- data.frame(
    pair_key = requested,
    gene_id = parse_gene_ids(requested),
    variant_id = sub("^[^_]*_", "", requested),
    stringsAsFactors = FALSE
  )
  # Repeated genes are allowed here: several variants of one gene can be
  # discovered. Variant IDs must still be unique because the genotype matrix is
  # keyed by variant.
  if (anyDuplicated(selected$variant_id)) {
    stop("The requested panel contains a duplicated variant ID.")
  }
} else {
  eligible_genes <- if (identical(panel, "threshold")) {
    names(gene_min_lfdr)[gene_min_lfdr > 0.90]
  } else {
    names(gene_min_lfdr)
  }
  set.seed(panel_seed)
  eligible_genes <- sample(eligible_genes, length(eligible_genes))
  selection <- select_random_unique_variant_per_gene(
    catalog,
    eligible_genes = eligible_genes,
    n_select = min(panel_size, length(eligible_genes)),
    set_id = panel
  )
  selected <- selection$selected
  if (nrow(selected) > panel_size) {
    selected <- selected[seq_len(panel_size), , drop = FALSE]
  }
}
selected$gene_min_lfdr <- unname(gene_min_lfdr[selected$gene_id])

selection_counts <- data.frame(
  panel = panel,
  n_selected = nrow(selected),
  n_unique_genes = length(unique(selected$gene_id)),
  median_gene_min_lfdr = stats::median(selected$gene_min_lfdr),
...
selection_list <- setNames(list(list(selected = selected)), panel)
selection_indices <- setNames(list(seq_len(nrow(unit_table))), panel)
...
null_generators <- c("donor_residual_block_permutation")
