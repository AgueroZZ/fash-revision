...
# Independently validate the saved five-seed R1 known-truth permutation MC.
...
summary_paths <- file.path(
  ...
  c(
    ...
    "source_reconstruction_validation.csv"
  )
)
if (any(!file.exists(c(replicate_paths, summary_paths)))) {
  stop("A required five-seed MC artifact is missing.")
}
...
if (nrow(saved_alpha005) != 20L || nrow(saved_mc_alpha005) != 4L ||
    nrow(source_validation) != 7L ||
    any(source_validation$maximum_absolute_difference >
          source_validation$tolerance)) {
  stop("A saved five-seed MC summary failed structural validation.")
}

recomputed_rows <- list()
row_index <- 0L
for (replicate in replicates) {
  if (!identical(replicate$seed, source_seeds[[row_index %/% 4L + 1L]]) ||
  ...
  unit_groups <- split(
    ...
    list(
      replicate$unit_results$arm,
      replicate$unit_results$fit_stage
    ),
    drop = TRUE
  )
  if (length(unit_groups) != 4L) {
    stop("A replicate does not contain four arm-stage unit groups.")
  }
...
if (!isTRUE(all.equal(recomputed, saved_comparison, tolerance = 1e-12))) {
  stop("Saved alpha-0.05 replicate results do not reproduce from unit lfdr.")
}
...
cat("R1 known-truth five-seed permutation MC validation passed.\n")
