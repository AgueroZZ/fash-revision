...
# Run and validate the full-universe R3 refresh under the canonical IWP1
# temporal mixture. The underlying driver remains resumable at the replicate
# level; this wrapper pins every scientific input and atomically promotes a
# validated partial cache to its final result directory.
...
EXPECTED_FUNCTIONAL_CANDIDATE_SCOPE <- "full_universe"
...
R3_DRIVER_PATH <- require_file(
  Sys.getenv(
    ...
    unset = file.path(
      ...
      "r3_full_universe_functional_fashr0143",
...
TEMPORAL_MIXTURE_CONTRACT <- r3_iwp1_temporal_mixture_contract(1272L)
if (!identical(
  TEMPORAL_CATEGORY_PROBS,
  TEMPORAL_MIXTURE_CONTRACT$temporal_category_probs
)) {
  stop(
    "The requested probabilities differ from the canonical IWP1 contract.",
    call. = FALSE
  )
}
...
if (!identical(
  ...
  stop("The formal R3 truth-cell counts differ from the mixture contract.",
       call. = FALSE)
}
...
log_message("Functional candidate scope: ", EXPECTED_FUNCTIONAL_CANDIDATE_SCOPE)
...
arguments <- c(
  ...
  "--candidate-scope", EXPECTED_FUNCTIONAL_CANDIDATE_SCOPE
)
...
if (!identical(configuration$output_id, RESULT_ID) ||
    ...
    !identical(
      configuration$functional_candidate_scope,
      EXPECTED_FUNCTIONAL_CANDIDATE_SCOPE
    ) ||
    !identical(configuration$functional_candidate_universe_size, J) ||
    !identical(
...
for (index in seq_along(replicates)) {
  ...
  if (!isTRUE(all.equal(replicate$configuration, configuration)) ||
      ...
      any(
        replicate$functional_alpha$candidate_scope !=
          EXPECTED_FUNCTIONAL_CANDIDATE_SCOPE
      ) ||
      any(replicate$functional_alpha$candidate_count != J) ||
      any(replicate$functional_alpha$first_stage_null_calls != 0L) ||
      any(!is.finite(replicate$functional_alpha$non_dynamic_calls)) ||
...
if (any(!is.finite(all_alpha$power)) ||
    ...
    any(all_alpha$candidate_scope != EXPECTED_FUNCTIONAL_CANDIDATE_SCOPE) ||
    any(all_alpha$candidate_count != J) ||
    any(all_alpha$first_stage_null_calls != 0L) ||
    any(!is.finite(all_alpha$non_dynamic_calls)) ||
    any(!is.finite(all_alpha_005$power)) ||
    any(!is.finite(all_pi0$estimated_pi0)) ||
    any(!is.finite(mc_alpha$mean_power)) ||
    any(mc_alpha$candidate_scope != EXPECTED_FUNCTIONAL_CANDIDATE_SCOPE) ||
    any(mc_alpha$mean_candidate_count != J) ||
...
middle_gate_rows <- mc_alpha[
  mc_alpha$target == "middle" &
    ...
    is.finite(mc_alpha$alpha) &
    mc_alpha$alpha >= 0.05 - 1e-12 &
    mc_alpha$alpha <= 0.20 + 1e-12,
  ,
  drop = FALSE
]
