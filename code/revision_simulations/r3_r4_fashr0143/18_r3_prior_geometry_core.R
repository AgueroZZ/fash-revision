...
# Run and validate the full-support, prior-geometry-matched R3 refresh.
# The underlying R3 driver remains resumable at the replicate level; this
# wrapper pins every scientific input and promotes a validated partial cache
# atomically to its final result directory.
...
EXPECTED_R3_DRIVER_SHA256 <-
  "32a0c6e3fb99e355d6c0ab0d68acbf7818b01d12f4516236addbfda2b33d48f9"
EXPECTED_SIMULATION_FUNCTIONS_SHA256 <-
  "3c5a97612591817a7edbbd3c331b380fae1d3a2fb468b4a78c0aed13ee3b8db9"
...
DEFAULT_RESULT_ID <- paste0(
  ...
  "iwp1_geometry_mixture_relative_clearance_main_effect_fashr0143_pilot5"
)
...
TEMPORAL_CATEGORY_PROBS <- parse_temporal_category_probs(
  Sys.getenv("FASH_R3_TEMPORAL_CATEGORY_PROBS", unset = "0.29,0.42,0.29"),
...
R3_DRIVER_PATH <- require_file(
  Sys.getenv(
    ...
    unset = file.path(
      WORKSPACE_DIR,
      "code", "revision_simulations", "r3_prior_geometry_fashr0143",
      "r3_prior_geometry_driver.R"
    )
  ),
  "R3 driver"
)
SIMULATION_FUNCTIONS_PATH <- require_file(
  Sys.getenv(
    ...
    unset = file.path(
      WORKSPACE_DIR,
      "code", "revision_simulations", "r3_prior_geometry_fashr0143",
      "r3_prior_geometry_simulation_functions.R"
    )
  ),
...
WRAPPER_CORE_PATH <- require_file(
  Sys.getenv(
    ...
    unset = file.path(
      WORKSPACE_DIR,
      "scripts", "18_r3_prior_geometry_core.R"
    )
  ),
  "R3 wrapper core"
)
...
log_message(
  paste(
    "Raised-cosine centers: early 1.5; middle 4.5-10.5; late 13.5.",
    "Each primary peak has complete support inside its target time region."
  )
)
...
arguments <- c(
  ...
  paste(TEMPORAL_CATEGORY_PROBS, collapse = ",")
)
...
manifest <- list(
  schema_version = "r3-fashr0143-manifest-v5-prior-geometry",
  result_id = RESULT_ID,
