...
# Run paired real-genotype functional-testing simulations under broad random
# B-spline and compact raised-cosine truth mechanisms. Each seed uses one
# paper-tested YRI dosage variant per gene from the shared R1/R2/R3 cache.
...
expected_truth_group_counts <- exact_balanced_counts(
...
make_replicate <- function(seed, truth_mechanism) {
  ...
  if (!identical(
    ...
    stop("The dynamic functional truth groups are not exactly balanced.")
  }
