...
simulate_matched_functional_effect_set <- function(
  ...
  required_center_groups <- c("early", "middle", "late")
  valid_center_ranges <- is.list(cosine_center_ranges) &&
    identical(names(cosine_center_ranges), required_center_groups) &&
    all(vapply(
      cosine_center_ranges,
      function(x) {
        is.numeric(x) &&
          length(x) == 2L &&
          all(is.finite(x)) &&
  ...
  if (!valid_center_ranges) {
    stop(
      paste(
        "cosine_center_ranges must contain increasing in-range numeric pairs",
        "named early, middle, and late."
      )
    )
  }
