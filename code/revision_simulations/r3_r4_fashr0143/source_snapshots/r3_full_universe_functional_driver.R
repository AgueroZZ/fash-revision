...
# Run the full-universe R3 functional calibration simulation under broad random
# B-spline and compact raised-cosine truth mechanisms. Each seed uses one
# paper-tested YRI dosage variant per gene from the shared cache.
...
functional_candidate_scope <- match.arg(
  get_arg("--candidate-scope", "full_universe"),
  c("dynamic_fdr_screen", "full_universe")
)
...
if (!identical(functional_candidate_scope, "full_universe")) {
  stop("The formal R3 calibration driver requires the full universe.")
}
...
if (preflight_only) {
  message(
    "Validated the full-universe R3 package, arguments, cache, and Middle definition: ",
    middle_expression,
    "."
  )
...
configuration <- list(
  ...
  non_switch_min_range_fraction =
    non_switch_min_range_fraction,
  functional_candidate_scope = functional_candidate_scope,
...
extract_call_diagnostics <- function(functional_result,
                                     ...
                                     seed,
                                     alpha = 0.05) {
  candidate_indices <- if (identical(
    functional_result$candidate_scope,
    "full_universe"
  )) {
  ...
  rows <- lapply(targets, function(target) {
    ...
    candidate_lfsr <- if (length(candidate_indices) == 0) {
      numeric()
    } else {
      unname(lfsr_map[as.character(candidate_indices)])
    }
...
validate_replicate <- function(x, seed, truth_mechanism) {
  ...
  all(required %in% names(x)) &&
    ...
    all(x$functional_alpha$candidate_scope == "full_universe") &&
    all(x$functional_alpha$candidate_count == J) &&
...
for (truth_mechanism in truth_mechanisms) {
  ...
  plot_mc_functional_curve_grid(
    ...
    title = paste("BF-adjusted empirical functional FSR:", truth_title),
