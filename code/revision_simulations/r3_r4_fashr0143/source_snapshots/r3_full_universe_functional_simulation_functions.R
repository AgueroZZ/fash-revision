...
temporal_middle_membership <- function(
    smooth_var,
    middle_window = c(4, 11),
    middle_boundary = c("closed", "open")) {
  smooth_var <- as.numeric(smooth_var)
  middle_window <- as.numeric(middle_window)
  middle_boundary <- match.arg(middle_boundary)
  if (length(smooth_var) < 2L || any(!is.finite(smooth_var))) {
    stop("smooth_var must contain at least two finite time points.")
  }
  if (length(middle_window) != 2L ||
      any(!is.finite(middle_window)) ||
      middle_window[[1L]] >= middle_window[[2L]]) {
    stop("middle_window must contain two finite, increasing endpoints.")
  }
  membership <- if (identical(middle_boundary, "closed")) {
    smooth_var >= middle_window[[1L]] & smooth_var <= middle_window[[2L]]
  } else {
    smooth_var > middle_window[[1L]] & smooth_var < middle_window[[2L]]
  }
  if (!any(membership) || all(membership)) {
    stop("The Middle definition must select a non-empty proper grid subset.")
  }
  membership
}

make_temporal_functionals <- function(
    smooth_var,
  ...
  list(
    ...
    middle = function(x) {
      x <- validate_curve(x)
      max(abs(x[middle_membership])) - max(abs(x[!middle_membership]))
    },
    late = function(x) {
      x <- validate_curve(x)
...
exact_temporal_truth_group_counts <- function(
    n,
    temporal_category_probs = NULL) {
  temporal_levels <- c("early", "middle", "late")
  if (is.null(temporal_category_probs)) {
    temporal_category_probs <- stats::setNames(
      rep(1 / length(temporal_levels), length(temporal_levels)),
      temporal_levels
    )
  }
  if (!identical(names(temporal_category_probs), temporal_levels) ||
      any(!is.finite(temporal_category_probs)) ||
      any(temporal_category_probs <= 0) ||
      abs(sum(temporal_category_probs) - 1) > 1e-8) {
    stop(paste(
      "temporal_category_probs must contain positive named probabilities",
      "for early, middle, and late that sum to one."
    ))
  }
  temporal_counts <- exact_proportional_counts(
    n,
    temporal_category_probs
  )
  truth_cells <- expand.grid(
    time_group = temporal_levels,
  ...
  group_counts <- stats::setNames(
    integer(nrow(truth_cells)),
    truth_cells$truth_group
  )
  for (time_group in temporal_levels) {
    labels <- paste(
      time_group,
      c("switch", "non-switch"),
      sep = " / "
    )
    group_counts[labels] <- exact_balanced_counts(
      temporal_counts[[time_group]],
      labels
    )
  }
  group_counts
...
assess_matched_functional_truth <- function(beta_evaluation,
  ...
  required_location_clearance <- max(
    location_truth_margin,
    location_truth_min_range_fraction * effect_range
  )
  other_time_groups <- setdiff(allowed_time_groups, time_group)
  timing_valid <-
    contrasts[[time_group]] >= required_location_clearance &&
    all(contrasts[other_time_groups] <= -required_location_clearance)
  ...
  list(
    ...
    required_location_clearance = required_location_clearance,
...
simulate_matched_functional_effect_set <- function(
  ...
  if (!identical(sort(names(class_probs)), sort(required_classes)) ||
      ...
      non_switch_min_range_fraction < 0 ||
      non_switch_min_range_fraction >= 1 ||
      (!is.null(temporal_category_probs) &&
        (!identical(
  ...
  required_location_clearance <- rep(NA_real_, n_variants)
  ...
  for (position in seq_along(dynamic_index)) {
    ...
    for (attempt in seq_len(max_attempts)) {
      ...
      required_location_clearance[index] <-
        assessment$required_location_clearance
  ...
  dynamic_competing_location_valid <- vapply(
    dynamic_index,
    function(index) {
      competing_groups <- setdiff(
        c("early", "middle", "late"),
        time_group[[index]]
      )
      all(true_functionals[index, competing_groups] <=
        -required_location_clearance[[index]])
    },
    logical(1)
  )
  if (any(dynamic_target_values <
        required_location_clearance[dynamic_index]) ||
      any(!dynamic_competing_location_valid) ||
  ...
  list(
    ...
    settings = list(
      ...
      raised_cosine_center_ranges = center_ranges,
      switch_threshold = switch_threshold,
      middle_window = middle_window,
      middle_boundary = middle_boundary,
      location_truth_margin = location_truth_margin,
      ...
      non_switch_min_range_fraction =
        non_switch_min_range_fraction,
      temporal_category_probs = if (is.null(temporal_category_probs)) {
        stats::setNames(rep(1 / 3, 3), c("early", "middle", "late"))
      } else {
        temporal_category_probs
      },
      exact_class_counts = exact_class_counts,
...
evaluate_fash_functional_testing <- function(fit,
  ...
  candidate_indices <- if (identical(candidate_scope, "full_universe")) {
    seq_len(nrow(true_functionals))
  } else {
    maximum_dynamic_indices
  }
  functionals <- make_temporal_functionals(
    smooth_var = evaluation_grid,
    switch_threshold = switch_threshold,
  ...
  for (alpha in alpha_grid) {
    ...
    alpha_candidate_indices <- if (identical(
      candidate_scope,
      "full_universe"
    )) {
      candidate_indices
    ...
    for (target in required_targets) {
      ...
      candidate_lfsr <- if (length(alpha_candidate_indices) == 0) {
        numeric()
      } else {
        unname(lfsr_map[as.character(alpha_candidate_indices)])
      }
      if (anyNA(candidate_lfsr)) {
        stop("Missing functional lfsr values for candidate variants.")
      }
      cfsr_table <- functional_cfsr_table(
        alpha_candidate_indices,
        candidate_lfsr
      )
      ...
      non_dynamic_calls <- if (discoveries == 0) 0L else {
        sum(!true_dynamic[selected_indices])
      }
      first_stage_null_calls <- if (identical(
        candidate_scope,
        "dynamic_fdr_screen"
      )) {
        non_dynamic_calls
      ...
      rows[[row_index]] <- data.frame(
        ...
        candidate_count = length(alpha_candidate_indices),
        ...
        non_dynamic_calls = non_dynamic_calls,
...
summarize_mc_functional_alpha_curves <- function(alpha_rows,
                                                 confidence_level = 0.95) {
  if (!"candidate_scope" %in% names(alpha_rows)) {
    alpha_rows$candidate_scope <- "dynamic_fdr_screen"
  }
  if (!"candidate_count" %in% names(alpha_rows)) {
    alpha_rows$candidate_count <- alpha_rows$dynamic_discoveries
  }
  if (!"non_dynamic_calls" %in% names(alpha_rows)) {
    alpha_rows$non_dynamic_calls <- alpha_rows$first_stage_null_calls
  }
  required_columns <- c(
    "seed", "scenario", "target", "method", "alpha", "candidate_scope",
    "candidate_count", "dynamic_discoveries",
    "n_discoveries", "false_discoveries", "conditional_discoveries",
    "conditional_false_discoveries", "non_dynamic_calls",
    "first_stage_null_calls", "true_positives",
  ...
  summaries <- lapply(groups, function(x) {
    if (length(unique(x$candidate_scope)) != 1L) {
      stop("Each functional summary group must have one candidate scope.")
    }
    metrics <- list(
      candidate_count = summarize_mc_values(x$candidate_count, confidence_level),
      ...
      non_dynamic_calls = summarize_mc_values(
        x$non_dynamic_calls,
        confidence_level
      ),
    ...
    data.frame(
      ...
      method = x$method[1],
      alpha = x$alpha[1],
      candidate_scope = x$candidate_scope[1],
      n_replications = length(unique(x$seed)),
      mean_candidate_count = metrics$candidate_count[["mean"]],
      ...
      mean_non_dynamic_calls = metrics$non_dynamic_calls[["mean"]],
...
plot_mc_functional_curve_grid <- function(mc_curve,
  ...
  if (!is.null(interval_columns)) {
    ...
    if (length(interval_columns) != 2L ||
        anyNA(interval_columns) ||
        any(!nzchar(interval_columns))) {
      stop("interval_columns must name exactly two non-empty columns.")
    }
    metric_columns[2:3] <- interval_columns
  }
  ...
  if (is.null(interval_methods)) {
    interval_methods <- styles$methods
  } else {
    interval_methods <- as.character(interval_methods)
    unknown_interval_methods <- setdiff(interval_methods, styles$methods)
    if (length(unknown_interval_methods) > 0L) {
      stop(
        "interval_methods contains unknown methods: ",
        paste(unknown_interval_methods, collapse = ", ")
      )
    }
  }
  if (is.null(x_limits)) {
    ...
    if (length(x_limits) != 2L ||
        any(!is.finite(x_limits)) ||
        x_limits[1] >= x_limits[2]) {
      stop("x_limits must contain two increasing finite values.")
    }
  }
  ...
  y_limits <- if (metric == "power") {
    ...
    interval_rows <- mc_curve$method %in% interval_methods
    upper <- max(
      c(
        x_limits[2],
        mc_curve[[value_column]],
        mc_curve[[upper_column]][interval_rows]
      ),
      na.rm = TRUE
    )
  ...
  for (target in target_order) {
    ...
    for (i in seq_along(styles$methods)) {
      ...
      if (styles$methods[i] %in% interval_methods &&
