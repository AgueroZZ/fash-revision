# Helpers for the R5 one-variant-per-gene sensitivity analysis.
...
select_one_variant_per_gene <- function(pair_keys,
                                        seed,
                                        gene_index = NULL) {
  ...
  if (anyDuplicated(selection$gene_id) ||
      anyDuplicated(selection$pair_key) ||
      any(selection$target_per_gene != 1L)) {
    stop("One-variant sampling did not return exactly one pair per gene.")
  }
  selection
}
...
summarize_gene_discovery_frequency <- function(discovered_gene_sets,
                                                seeds,
                                                all_genes,
                                                full_discovered_genes) {
  seeds <- as.integer(seeds)
  all_genes <- sort(unique(as.character(all_genes)), method = "radix")
  ...
  if (!is.list(discovered_gene_sets) ||
      ...
      length(all_genes) < 2L || anyNA(all_genes) || any(!nzchar(all_genes)) ||
      anyNA(full_discovered_genes) || any(!nzchar(full_discovered_genes)) ||
      any(!full_discovered_genes %in% all_genes)) {
    stop("Gene-discovery frequency inputs are invalid or misaligned.")
  }
  discovered_gene_sets <- lapply(discovered_gene_sets, function(genes) {
    genes <- unique(as.character(genes))
    if (anyNA(genes) || any(!nzchar(genes)) || any(!genes %in% all_genes)) {
      stop("Every discovered-gene set must use valid genes from all_genes.")
    }
    genes
  })
  discovery_counts <- table(factor(
    unlist(discovered_gene_sets, use.names = FALSE),
    levels = all_genes
  ))
  result <- data.frame(
    gene_id = all_genes,
    n_seeds_discovered = as.integer(discovery_counts),
    discovery_frequency = as.integer(discovery_counts) / length(seeds),
    full_data_discovered_gene = all_genes %in% full_discovered_genes,
    stringsAsFactors = FALSE,
    row.names = NULL
  )
  if (nrow(result) != length(all_genes) ||
      any(result$n_seeds_discovered < 0L) ||
      any(result$n_seeds_discovered > length(seeds)) ||
      any(result$discovery_frequency < 0 | result$discovery_frequency > 1)) {
    stop("Gene-discovery frequencies failed validation.")
  }
  result
}
