...
all_genes_files <- list.files(path = "./eQTL_results_CL/") # for cell-line PC based result
all_genes <- gsub(pattern = "^X|.rds", "", all_genes_files)

datasets <- list()
for (gene in all_genes) {
  dataset <- readRDS(paste0("./eQTL_results_CL/", gene, ".rds"))
...
save(datasets, file = "./results/datasets_corrected_CL.RData")


load("./results/datasets_corrected_CL.RData")
...
save(fash_fit1, file = "./results/fash_fit1_all_CL.RData")

fash_fit2 <- fash(Y = "beta", smooth_var = "time", S = "SE", data_list = datasets,
                  num_basis = 20, order = 2, betaprec = 0,
                  pred_step = 1, penalty = 10, grid = fine_grid,
                  num_cores = num_cores, verbose = TRUE)
save(fash_fit2, file = "./results/fash_fit2_all_CL.RData")
