...
# PC_data <- read.delim("./data/principal_components_10.txt") # timepoint specific PCs
PC_data <- read.delim("./data/cell_line_ignore_missing_principal_components_9.txt") # cell line specific PCs
...
analyze_one_gene <- function(selected_gene){
  ...
  for (selected_variant in all_relevant_variants) {
    ...
    for (selected_time in all_time) {
      ...
      # PC_selected <- PC_data[paste0(selected_cell_lines, "_", selected_time), c("PC1", "PC2", "PC3", "PC4", "PC5")] # timepoint specific PCs
      PC_selected <- PC_data[selected_cell_lines, c("PC1", "PC2", "PC3", "PC4", "PC5")] # cell line specific PCs
...
if(num_cores == 1){
  ...
  mclapply(all_unique_genes, function(selected_gene) {
    if (!file.exists(paste0("./eQTL_results_CL/", selected_gene, ".rds"))) {
      cat("Analyzing gene ", selected_gene, "\n")
      result <- analyze_one_gene(selected_gene)
      saveRDS(result, paste0("./eQTL_results_CL/", selected_gene, ".rds"))
    }else{
      cat("Result already exists, skipping gene ", selected_gene, "\n")
    }
  }, mc.cores = num_cores)
