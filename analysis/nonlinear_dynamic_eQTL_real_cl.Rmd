---
title: "Nonlinear Dynamic eQTL analysis on iPSC (CL-PC)"
author: "Ziang Zhang"
date: "2025-02-21"
output: 
...
plot_many_pairs_nonlinear <- function(idxs, nrow = 2, ncol = 2, ...) {
  ...
  idxs <- idxs[is.finite(idxs) & idxs >= 1]
  if (length(idxs) == 0L) return(invisible(NULL))
...
```{r}
load(file.path(raw_result_dir, "fash_fit2_all_CL.RData"))
```
...
save(fash_fit2_update, file = file.path(result_dir, "fash_fit2_update_CL.RData"))
```

```{r echo=FALSE}
stopifnot(as.character(packageVersion("fashr")) == "0.1.43")
loaded_fit2_update <- load(file.path(result_dir, "fash_fit2_update_CL.RData"))
stopifnot(
  identical(loaded_fit2_update, "fash_fit2_update"),
  inherits(fash_fit2_update, "fash")
)
rm(loaded_fit2_update)
```

All BF-adjusted results below use the retained CL-PC fit recomputed with
`fashr` 0.1.43. The conservative estimate is
`r fash_fit2_update$prior_weights$prior_weight[1]`.
...
genes_highlighted2_subset_ens <- head(genes_highlighted2, 4)
...
par(mfrow = c(2, 2))
for (i in seq_len(min(4, length(gene_fashr_only)))) {
...
par(mfrow = c(2, 2))
for (i in seq_len(min(4, length(gene_missed)))) {
...
The retained classification files were recomputed from `fash_fit2_update` using one shared set of 3,000 posterior draws per pair across the four functionals. The canonical regeneration entrypoint is `code/03_nonlindyn_lfsr.R`. We first take a look at the significant pairs detected by FASH (order 2), and classify them based on the false sign rate (lfsr):
...
```{r}
load(file.path(classification_dir, "classify_nonlin_dyn_eQTLs_middle.RData"))
...
```{r}
load(file.path(classification_dir, "classify_nonlin_dyn_eQTLs_late.RData"))
...
```{r}
load(file.path(classification_dir, "classify_nonlin_dyn_eQTLs_switch.RData"))
