...
result_all <- lapply(pho_vec, function(pho0){
  pho1 <- pho0 / 2
  get_result_once(J = 1000, pho0 = pho0, pho1 = pho1, sigma_vec = c(0.1, 0.3, 0.5),
...
We will fix $\rho_0 = 0.2$ and $\rho_1 = 0.1$ (the `pho0` and `pho1` arguments below), and focus on one particular replication to illustrate the inference using FASH. These are the proportions of *dynamic* and *nonlinear-dynamic* units, so the corresponding true null proportions are $\pi_0 = 1 - \rho_0 = 0.8$ when testing $S_0$ as constant functions (IWP$_1$) and $\pi_0 = 1 - \rho_1 = 0.9$ when testing $S_0$ as linear functions (IWP$_2$).
