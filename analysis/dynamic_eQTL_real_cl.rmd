---
title: "Dynamic eQTL analysis on iPSC (CL-PC)"
author: "Ziang Zhang"
date: "2025-02-21"
output: 
...
classification_dir <- file.path(result_dir, "dynamic_classification")
...
This CL-PC sensitivity analysis estimates the first 5 PCs after collapsing
expression to the cell-line level, then repeats those PC values across time.
...
```{r}
load(file.path(raw_result_dir, "fash_fit1_all_CL.RData"))
```

```{r}
# This retained object uses the CL-based PCs described above.
```
...
gene_map <- get_gene_map(
  ens_ids = all_genes,
  cache_path = file.path(raw_result_dir, "cache_gene_map.rds")
)
```
...
One likely reason could be model misspecification under the alternative hypothesis.
We therefore compute a conservative estimate of $\pi_0$ using the BF procedure:

```{r eval=FALSE}
fash_fit1_update <- BF_update(fash_fit1, plot = FALSE)
fash_fit1_update$prior_weights
save(fash_fit1_update, file = file.path(result_dir, "fash_fit1_update_CL.RData"))
```

```{r echo=FALSE}
stopifnot(as.character(packageVersion("fashr")) == "0.1.43")
loaded_fit1_update <- load(file.path(result_dir, "fash_fit1_update_CL.RData"))
stopifnot(
  identical(loaded_fit1_update, "fash_fit1_update"),
  inherits(fash_fit1_update, "fash")
)
rm(loaded_fit1_update)
```

All BF-adjusted results below use the retained CL-PC fit recomputed with
`fashr` 0.1.43. The conservative estimate is
`r fash_fit1_update$prior_weights$prior_weight[1]`.
...
The remainder of the analysis uses the cumulative-lfdr discoveries from the BF-adjusted fit.
...
map_sym2ens <- symbol_to_ens(
  ...
  cache_path = file.path(raw_result_dir, "cache_sym2ens_subset.rds")
)
...
gene_sets <- list(
  ...
  "FASH (1; CL-PC)" = genes_highlighted1
)
...
pair_sets <- list(
  ...
  "FASH (1; CL-PC)" = pairs_highlighted1
)
...
gene_sets_unadj <- list(
  "Strober (Nonlinear)" = genes_highlighted_strober_nonlinear,
  "Strober (Linear)" = genes_highlighted_strober_linear,
  "FASH (1; CL-PC) unadj" = genes_highlighted1_before
)
...
pair_sets_unadj <- list(
  "Strober (Nonlinear)" = pairs_highlighted_strober_nonlinear,
  "Strober (Linear)" = pairs_highlighted_strober_linear,
  "FASH (1; CL-PC) unadj" = pairs_highlighted1_before
)
...
sym2ens2 <- head(genes_pass_filter, 4)
selected_gene_fashr_only <- symbol_of(sym2ens2, gene_map)
...
gene_ens2 <- head(genes_pass_filter2, 4)
selected_gene_fashr_only <- symbol_of(gene_ens2, gene_map)
...
```{r}
select_gene_id <- gene_ens2
select_variant_id <- pair_tbl$rs_id[match(idxs2, pair_tbl$idx)]
...
plot_many_pairs_lfsr <- function(idxs, testing_obj, nrow = 2, ncol = 2, ...) {
  idxs <- as.integer(idxs)
  idxs <- idxs[is.finite(idxs) & idxs >= 1L]
  if (length(idxs) == 0L) {
    return(invisible(NULL))
  }
...
```{r}
load(file.path(classification_dir, "classify_dyn_eQTLs_early.RData"))
...
```{r}
load(file.path(classification_dir, "classify_dyn_eQTLs_middle.RData"))
...
```{r}
load(file.path(classification_dir, "classify_dyn_eQTLs_late.RData"))
...
genes_use <- tail(late_genes, min(4L, length(late_genes)))
...
```{r}
load(file.path(classification_dir, "classify_dyn_eQTLs_switch.RData"))
...
enrichment_table(result, pvalue_cutoff = 0.05)
```
